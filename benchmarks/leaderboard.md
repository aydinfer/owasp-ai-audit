...
| **vercel/ai-chatbot** *(re-audited under v1.0.0)* | 🟠 Concerning | 🟡 | 🟡 | 🟢 | 🟡 | 🟢 | 🟡 | 0 / 2 / 24 / 23 / 0 / 48 | [dashboard](./vercel-ai-chatbot/dashboard.html) |
...
- **vercel/ai-chatbot — 🟠 Concerning** *(re-audited under v1.0.0; the other four rows are v0.2.x screens).* The v0.2.2 run graded only 26 of 97 taxonomy entries and still labelled it "Acceptable" — the dishonesty pattern v1.0.0 exists to kill. The v1.0.0 re-audit adjudicates **all 97 entries** (85 applicable + 12 justified predictive N/A) at measured 100% coverage on all eight layers; the honest rollup of four AMBER categories is **Concerning**. A deep reasoned-probe pass then earned **2 HIGH** the honest way — uncounted nested `streamText` cost-amplification (the `role:'user'` rate counter misses tool-spawned LLM calls) and always-on geolocation injection — each with a verbatim probe (L5 = 2/2). Most other locks **held** (IDOR re-checked per route, tools re-derive authority from the session, model selection allowlisted) and were graded down accordingly; 0 CRITICAL because the app wasn't run. See [v0.2.2-vs-v1.0.0.md](./v0.2.2-vs-v1.0.0.md). → [report](./vercel-ai-chatbot/dashboard.html)
