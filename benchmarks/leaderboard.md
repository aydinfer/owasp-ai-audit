...
| **vercel/ai-chatbot** | 🟡 Acceptable | 🟢 | 🟡 | 🟢 | 🟢 | 🟢 | 🟢 | 0 / 1 / 6 / 5 / 1 / 13 | [dashboard](./vercel-ai-chatbot/dashboard.html) |
...
- **vercel/ai-chatbot — 🟡 Acceptable.** Well-engineered Next.js chat template with strong conventional security (per-row authz, parameterised Drizzle, bcrypt, sanitising markdown, client-side Pyodide sandbox), but a **fail-open / guest-bypassable rate limiter (HIGH)**, an undefended prompt-injection path into low-privilege user-scoped tools (MEDIUM), and privacy-retention + AI-testing gaps. → [report](./vercel-ai-chatbot/dashboard.html)
