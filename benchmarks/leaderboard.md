...
| **vercel/ai-chatbot** *(re-audited under v1.0.0)* | 🟠 Concerning | 🟡 | 🟡 | 🟢 | 🟡 | 🟢 | 🟢 | 0 / 0 / 26 / 23 / 0 / 48 | [dashboard](./vercel-ai-chatbot/dashboard.html) |
...
- **vercel/ai-chatbot — 🟠 Concerning** *(re-audited under v1.0.0; the other four rows are v0.2.x screens).* The v0.2.2 run on this repo graded only 26 of 97 taxonomy entries and still labelled it "Acceptable" — the dishonesty pattern v1.0.0 exists to kill. The v1.0.0 re-audit adjudicates **all 97 entries** (85 applicable + 12 justified predictive N/A) at measured 100% coverage on all eight layers, and the honest rollup of three AMBER categories is **Concerning**, not Acceptable. Every finding is `static`-class and capped at MEDIUM (no probes run); the v0.2.2 "rate-limiter HIGH" is gone — `lib/ratelimit.ts` does rate-limit, so it was both unprobed and wrong. See [v0.2.2-vs-v1.0.0.md](./v0.2.2-vs-v1.0.0.md). → [report](./vercel-ai-chatbot/dashboard.html)
